                       ...
                       uint8_t numberOfStreams)
{
    ...
    phy->SetCam (cam);
    phy->SetDevice (dev);
    phy->SetChannel (bwp->m_channel); //zhongwei: channel is needed to be connected to the GnbPhy to start transmission StartMultiTx
...
NrHelper::AttachToClosestEnb(NetDeviceContainer ueDevices, NetDeviceContainer enbDevices)
{
    ...
    {
        int index_ue = i - ueDevices.Begin();
        //pengpeng:
        AttachToClosestEnb(*i, enbDevices, index_ue);
    }
}
...
//pengpeng: use index_ue to do antenna split
void
NrHelper::AttachToClosestEnb(Ptr<NetDevice> ueDevice, NetDeviceContainer enbDevices, int index_ue)
{
    NS_LOG_FUNCTION(this);
    NS_ASSERT_MSG(enbDevices.GetN() > 0, "empty enb device container");
    Vector uepos = ueDevice->GetNode()->GetObject<MobilityModel>()->GetPosition();
    ...
    AttachToEnb(ueDevice, closestEnbDevice, index_ue);
}

void
NrHelper::AttachToEnb(const Ptr<NetDevice>& ueDevice, const Ptr<NetDevice>& gnbDevice, int index_ue)
{
    Ptr<NrGnbNetDevice> enbNetDev = gnbDevice->GetObject<NrGnbNetDevice>();
    Ptr<NrUeNetDevice> ueNetDev = ueDevice->GetObject<NrUeNetDevice>();
    ...
    if (m_beamformingHelper)
    {
        m_beamformingHelper->AddBeamformingTask(enbNetDev, ueNetDev, index_ue);
    }
}
