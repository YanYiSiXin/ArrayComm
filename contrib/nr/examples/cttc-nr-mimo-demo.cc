 ...
 *
 * \code{.unparsed}
$ ./ns3 run "cttc-nr-mimo-demo --PrintHel
    \endcode
 *
 */
...
std::ofstream m_outpathLossFile;
std::ofstream m_outshadowingFile; // ShadowingEnabled be true to produce shadowing value
std::ofstream m_outrxDataFile;
std::ofstream m_outUePositionsFile;
std::ofstream m_outGnbPositionsFile;
std::ofstream m_outstreamFile;
void UeStreamTrace(uint8_t rx_streamid, uint8_t tx_streamid, Ptr<const SpectrumValue> rxpsd, Ptr<const SpectrumValue> noise)
{
    if (rx_streamid == tx_streamid)
    {
        m_outstreamFile << "data:   " << rx_streamid+0 << "    " << tx_streamid+0 <<"    "<< 10*log10(Sum(*rxpsd)) <<"dB     " << 10*log10(Sum(*noise)) <<"dB " ;
    }
    else
    {
        m_outstreamFile << "int:    " << rx_streamid+0 << "    " << tx_streamid+0 <<"    "<< 10*log10(Sum(*rxpsd)) <<"dB     " << 10*log10(Sum(*noise)) <<"dB " ;
    }
    m_outstreamFile << std::endl;
}
void UeReceptionTrace (RxPacketTraceParams params)
{
    m_outSinrFile << params.m_cellId << " " << params.m_rnti << "\t" << 10 * log10 (params.m_sinr) << std::endl;
}
void AllReceptionTrace (const SfnSf &current_sfn, Ptr<const SpectrumValue> rxpsd, const Time &duration, uint16_t phy_bwpid, uint16_t phy_cellid)
{
    m_outrxDataFile << current_sfn << " " << Sum(*rxpsd) << " " << duration << " " << phy_bwpid << " " << phy_cellid << std::endl;
}
void PathlossTrace (Ptr<const SpectrumPhy> txPhy, Ptr<const SpectrumPhy> rxPhy, double pl)
{
    ...
    if (DynamicCast<NrGnbNetDevice> (txNrSpectrumPhy->GetDevice ()) != nullptr)
    {
        if (DynamicCast<NrUeNetDevice> (rxNrSpectrumPhy->GetDevice ()) != nullptr)
        {
            ...
            m_outpathLossFile << txCellId << " " << txBwpId << " " << txStreamId << " " << rxCellId << " " << rxBwpId << " " << rxStreamId << " " << pl << std::endl;
        }
    }
}
void ShadowingTrace (Ptr<MobilityModel> a, Ptr<MobilityModel> b, double sh)
{
    m_outshadowingFile << a << " " << b << " " << sh << std::endl;
}
int
main(int argc, char* argv[])
{
    //xiaoying : Measure real simulation time
    float start ;
    start = std::clock();
    /*
    ...
    //pengpeng: 4x4 mimo schme
    uint16_t numRowsGnb = 4;
    uint16_t numColumnsGnb = 2;
    uint16_t numRowsUe = 1;
    uint16_t numColumnsUe = 1;
    ...
    //pengpeng: distance from gnb to the 2th ue
    uint16_t gnbUeDistance_1 = 20;

    // Simulation parameters. Please don't use double to indicate seconds; use
    // ns-3 Time values which use integers to avoid portability issues.
    //xiaoying : Configure up and down link simulation time and send time respectively
    Time simTime = MilliSeconds (500);
    Time udpAppStartTimeDl = MilliSeconds (400);
    Time udpAppStartTimeUl = MilliSeconds (400);
    Time udpAppStopTimeDl = simTime;   // Let's give 2s to end the tx
    Time udpAppStopTimeUl = simTime;   // Let's give 2s to end the tx
    ...
    cmd.AddValue ("InterStreamInterferenceRatio",
                "zhongwei: determine the ratio of the inter-stream interference",
                InterStreamInterferenceRatio);
    cmd.AddValue ("UseRawDlData",
                "zhongwei: determine to use XNEW scheme or LENA scheme",
                UseRawDlData);
    cmd.AddValue("m_isUplink",
                "xiaoying : Enable Uplink",
                m_isUplink);
    cmd.AddValue("m_isDownlink",
                "xiaoying : Enable Downlink",
                m_isDownlink);
    cmd.AddValue("useAWGN",
                "xiaoying : use AWGN channel",
                useAWGN);
    // Parse the command line
    cmd.Parse(argc, argv);

    std::string resultsPath = "./contrib/nr/examples/";
    std::string outFile_stream_trace = resultsPath + "stream_trace" ;
    std::string outFile_Sinr = resultsPath + "sinrs";
    std::string outFile_rxData = resultsPath + "txdata";
    std::string outFile_pathloss = resultsPath + "pathloss";
    std::string outFile_shadowing = resultsPath + "shadowing";
    std::string outFile_UePositions = resultsPath + "ue-positions";
    std::string outFile_GnbPositions = resultsPath + "gnb-positions";
    m_outSinrFile.open (outFile_Sinr.c_str ());
    m_outSinrFile.setf (std::ios_base::fixed);
    if (!m_outSinrFile.is_open ())
    {
        NS_ABORT_MSG ("Can't open file " << outFile_Sinr);
    }
    m_outrxDataFile.open (outFile_rxData.c_str ());
    m_outrxDataFile.setf (std::ios_base::fixed);
    m_outrxDataFile.precision(16);
    if (!m_outrxDataFile.is_open ())
    {
        NS_ABORT_MSG ("Can't open file " << outFile_rxData);
    }
    m_outpathLossFile.open (outFile_pathloss.c_str ());
    m_outpathLossFile.setf (std::ios_base::fixed);
    if (!m_outpathLossFile.is_open ())
    {
        NS_ABORT_MSG ("Can't open file " << outFile_pathloss);
    }
    m_outshadowingFile.open (outFile_shadowing.c_str ());
    m_outshadowingFile.setf (std::ios_base::fixed);
    if (!m_outshadowingFile.is_open ())
    {
        NS_ABORT_MSG ("Can't open file " << outFile_shadowing);
    }
    m_outUePositionsFile.open (outFile_UePositions.c_str ());
    m_outUePositionsFile.setf (std::ios_base::fixed);
    if (!m_outUePositionsFile.is_open ())
    {
        NS_ABORT_MSG ("Can't open file " << outFile_UePositions);
    }
    m_outGnbPositionsFile.open (outFile_GnbPositions.c_str ());
    m_outGnbPositionsFile.setf (std::ios_base::fixed);
    if (!m_outGnbPositionsFile.is_open ())
    {
        NS_ABORT_MSG ("Can't open file " << outFile_GnbPositions);
    }
    /*
 ...
 /* //pengpeng: 4x4 MIMO scheme
    ...
    positionAllocUe->Add(Vector(0.0, 0.0, 10.0));
    positionAllocUe->Add(Vector(gnbUeDistance, 0.0, 1.5));
    //pengpeng: the pos of the second UE and install it using mobility class
    positionAllocUe->Add(Vector(gnbUeDistance_1, 0.0, 2));
    mobility.SetPositionAllocator(positionAllocUe);
    mobility.Install(gnbContainer);
    mobility.Install(ueContainer);
    ...
    nrHelper->SetSchedulerAttribute ("UseRawFSlotStructure", BooleanValue (UseRawDlData));
    nrHelper->SetUePhyAttribute ("UseRawCqiMethod", BooleanValue (UseRawDlData));
    nrHelper->SetUeSpectrumAttribute ("UseRawRbSymToCqi", BooleanValue (UseRawDlData));
  ...
  nrHelper->SetUeSpectrumAttribute ("InterStreamInterferenceRatio", DoubleValue (InterStreamInterferenceRatio));
    ...
    { //   The second subarray
        ...
        nrSpectrumPhy->GetAntenna()->GetObject<UniformPlanarArray>()->SetAttribute(
            ...
            DoubleValue(polarizationSecondSubArray));
        // The third subarray
        nrSpectrumPhy = gnbSpectrumPhys.Get(2)->GetObject<NrSpectrumPhy>();
        nrSpectrumPhy->GetAntenna()->GetObject<UniformPlanarArray>()->SetAttribute(
            "PolSlantAngle",
            DoubleValue(polarizationFirstSubArray));
        // The fourth subarray
        nrSpectrumPhy = gnbSpectrumPhys.Get(3)->GetObject<NrSpectrumPhy>();
        nrSpectrumPhy->GetAntenna()->GetObject<UniformPlanarArray>()->SetAttribute(
            "PolSlantAngle",
            DoubleValue(polarizationSecondSubArray));
    }
    ...
    //         DoubleValue(polarizationSecondSubArray));
    // }
    if (ueSpectrumPhys.GetN() == 4)
    {   // pengpeng: The second subarray
        nrSpectrumPhy = ueSpectrumPhys.Get(1)->GetObject<NrSpectrumPhy>();
        nrSpectrumPhy->GetAntenna()->GetObject<UniformPlanarArray>()->SetAttribute(
            "PolSlantAngle",
            DoubleValue(polarizationSecondSubArray));
        // pengpeng: The third subarray
        nrSpectrumPhy = ueSpectrumPhys.Get(2)->GetObject<NrSpectrumPhy>();
        nrSpectrumPhy->GetAntenna()->GetObject<UniformPlanarArray>()->SetAttribute(
            "PolSlantAngle",
            DoubleValue(polarizationFirstSubArray));
        // pengpeng: The fourth subarray
        nrSpectrumPhy = ueSpectrumPhys.Get(3)->GetObject<NrSpectrumPhy>();
        nrSpectrumPhy->GetAntenna()->GetObject<UniformPlanarArray>()->SetAttribute(
            "PolSlantAngle",
            DoubleValue(polarizationSecondSubArray));
    }
    ...
    //xiaoying : in this container, interface 0 is the pgw, 1 is the remoteHost
   ...
   */
    ...
    Ptr<NrUePhy> uephy = DynamicCast<NrUeNetDevice>(ueNetDev.Get (0))->GetPhy (0);
    //uephy->TraceConnectWithoutContext ("Rsrp", MakeCallback (&)); only array0(stream0) ctrl signal will calc rsrp
    for (uint32_t i = 0; i < subArraysUe; i++)
    {
        Ptr<NrSpectrumPhy > ueSpectrumPhy = uephy->GetSpectrumPhy (i);
        ueSpectrumPhy->TraceConnectWithoutContext ("RxPacketTraceUe", MakeCallback (&UeReceptionTrace));
        ueSpectrumPhy->TraceConnectWithoutContext ("RxDataTrace", MakeCallback (&AllReceptionTrace));
        ueSpectrumPhy->TraceConnectWithoutContext ("HbfStreamTrace", MakeCallback (&UeStreamTrace));
        ueSpectrumPhy->GetSpectrumChannel()->TraceConnectWithoutContext ("PathLoss", MakeCallback (&PathlossTrace));
        Ptr<ThreeGppPropagationLossModel> tgppLargescaleloss = DynamicCast<ThreeGppPropagationLossModel>(ueSpectrumPhy->GetSpectrumChannel()->GetPropagationLossModel());
        tgppLargescaleloss->TraceConnectWithoutContext ("shadowing", MakeCallback (&ShadowingTrace));
    }
    ...
    float TimeValue;
    TimeValue=end - start;
    std::cout<<"real TimeValue : "<<TimeValue/CLOCKS_PER_SEC<<" s "<<std::endl;
    ...
    */
    ...
    std::string filename = resultsPath + simTag;
    ...
    Simulator::Destroy ();
    m_outSinrFile.close ();
    m_outrxDataFile.close ();
    m_outpathLossFile.close ();
    m_outshadowingFile.close ();
    m_outstreamFile.close ();
    return 0;
}
