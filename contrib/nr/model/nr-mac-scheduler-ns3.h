...
class NrMacSchedulerNs3 : public NrMacScheduler
{
  ...
  /**
  * \xiaoying:brief whether to use multi-data scheme.
  * \multi-data scheme api
  */
    void SetUseRawFSlotStructure (bool UseRawFSlotStructure);
  ...
  private:
    ...
    uint8_t DoScheduleDl(const std::vector<DlHarqInfo>& dlHarqFeedback,
                         ...
                         LteNrTddSlotType type);//xiaoying:Add function input parameter “Slot type”
  ...
  private:
    ...
    bool m_UseRawFSlotStructure {true} ; //xiaoying:If m_UseRawFSlotStructure is true, LENA scheme the original symbol distribution of timeslot F is maintained; otherwise, timeslot F becomes an XNEW design scheme
