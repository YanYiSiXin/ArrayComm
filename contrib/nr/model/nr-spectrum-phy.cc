...
NrSpectrumPhy::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NrSpectrumPhy")
            ...
            .AddAttribute ("UseRawRbSymToCqi",
                            "bug3:If UseRawRbSymToCqi is true, the number of symbols corresponding " 
                            " to RB number when calculating CQI is the number of DATA type symbols that match the current time slot. Otherwise, not.",
                            BooleanValue (true),
                            MakeBooleanAccessor (&NrSpectrumPhy::SetUseRawRbSymToCqi),
                            MakeBooleanChecker ())
            ...
            .AddTraceSource("DlDataPathloss",
                            ...
                            "ns3::NrSpectrumPhy::DlPathlossTrace")
            .AddTraceSource ("HbfStreamTrace",
                             "Trace hbf received stream psd(power)",
                             MakeTraceSourceAccessor (&NrSpectrumPhy::m_hbfStreamTrace),
                             "ns3::NrSpectrumPhy::StreamTracedCallback");
...
void
NrSpectrumPhy::SetUseRawRbSymToCqi(bool UseRawRbSymToCqi)
{
    NS_LOG_FUNCTION (this << UseRawRbSymToCqi);
    m_UseRawRbSymToCqi = UseRawRbSymToCqi;
}
void
...
NrSpectrumPhy::StartRx(Ptr<SpectrumSignalParameters> params)
{
    ...
    Ptr <const SpectrumValue> noisePsd = m_interferenceData->GetNoisePowerSpectralDensity();
    if (nrDataRxParams)
    {
            ...
            nrDataRxParams->txPhy->GetObject<NrSpectrumPhy>()->GetStreamId() != m_streamId)
        {
            ...
            uint8_t streamid = nrDataRxParams->txPhy->GetObject<NrSpectrumPhy> ()->GetStreamId ();
            m_hbfStreamTrace(m_streamId, streamid, rxPsd, noisePsd);
            return;
        }
    }
                                 ...
                                 Time duration)
{
    ...
    case IDLE: {
        ...
        txParams->streamId = m_streamId;
        txParams->type = 0;
        txParams->isOneStream = true;
...
NrSpectrumPhy::StartTxMultiDataFrames (const Ptr<PacketBurst>& pb, const std::list<Ptr<NrControlMessage> >& ctrlMsgList,
                                      Time duration)
{
  NS_LOG_FUNCTION (this);
  ...
  switch (m_state)
    ...
    Ptr<SpectrumSignalParameters> tempParams = txParams;
    //zhongwei: return each stream's txParams to NrGnbPhy
    return tempParams;
}

void
NrSpectrumPhy::EndMultiDataTx (Time duration)
{
  Simulator::Schedule (duration, &NrSpectrumPhy::EndTx, this);
}
...
NrSpectrumPhy::GenerateDlCqiReport(const SpectrumValue& sinr)
{
    ...
    NS_ABORT_MSG_UNLESS(
        phy,
        "This function should only be called for NrSpectrumPhy belonging to NrUEPhy");
    if (m_UseRawRbSymToCqi)
    {
      phy->GenerateDlCqiReport (sinr, m_streamId);
    }
    else
    {
      phy->GenerateDlCqiReportNew (sinr, m_streamId, m_symnum);
    }
    ...
    case TX:
                     ...
                     // the same time as me. -> invalid state.
        {
            NS_FATAL_ERROR("eNB transmission overlaps in time with UE transmission. CellId:"
             ...
             // transmitting to gNB).
        {
            //renew one stream nosie
            //std::cout << params->type +0 << " " << params->isOneStream << std::endl;
            if (params->type == 0 && params->isOneStream)
            {
                m_interferenceData->SetNoisePowerSpectralDensity (m_phy->GetNoisePowerSpectralDensity ());
            }
...
NrSpectrumPhy::EndRxData()
{
    ...
    for (auto& tbIt : m_transportBlocks)
    {
        ...
        /*if (!m_isEnb)
        {
            std::cout<<"ue "<<"stream " << (uint32_t)m_streamId <<" sfn "<<GetTBInfo(tbIt).m_expected.m_sfn << " sinr " << m_sinrPerceived[0]
            <<" bitmap " <<GetTBInfo (tbIt).m_expected.m_rbBitmap.size ()
            << " m_tbler: " << GetTBInfo(tbIt).m_outputOfEM->m_tbler+0 
            << " corrupted " << GetTBInfo(tbIt).m_isCorrupted
            <<	" RNTI " << GetRnti (tbIt) << " processId " <<
            +GetTBInfo(tbIt).m_expected.m_harqProcessId << " size " <<
            GetTBInfo (tbIt).m_expected.m_tbSize <<  " mcs " << (uint32_t)GetTBInfo (tbIt).m_expected.m_mcs << " rv " << +GetTBInfo (tbIt).m_expected.m_rv << std::endl;
        }
        else
        {
            std::cout<<"gnb " <<"stream " << (uint32_t)m_streamId <<" sfn "<<GetTBInfo(tbIt).m_expected.m_sfn << " sinr " << m_sinrPerceived[0]
            <<" bitmap " <<GetTBInfo (tbIt).m_expected.m_rbBitmap.size ()
            << " m_tbler: " << GetTBInfo(tbIt).m_outputOfEM->m_tbler+0
            << " corrupted " << GetTBInfo(tbIt).m_isCorrupted
            <<	" RNTI " << GetRnti (tbIt) << " processId " <<
