...
void
NrGnbPhy::InitPrecodingMatrix()
{
    // complex2DVector_t precodingmatrix(2, complexVector_t(2, std::complex<double>(0.5, 0.0)));
    // m_precodingmatrix = precodingmatrix;
    // std::ifstream ifs("contrib/nr/examples/precodingmatrix.txt", std::ios::in);
    // if (ifs)
    // {
    //     double a, b;
    //     int i = 0;
    ...
    //     {
    //         int j = 0;
    //         m_precodingmatrix[i][j++] = a;
    //         m_precodingmatrix[i][j++] = b;
    //         i++;
    //     }
    //     ifs.close();
    // }
    //pengpeng: 4x4 mimo scheme
    complex2DVector_t precodingmatrix(4, complexVector_t(4, std::complex<double>(0.5, 0.0)));
    m_precodingmatrix = precodingmatrix;
    std::ifstream ifs("contrib/nr/examples/precodingmatrix.txt", std::ios::in);
    if (ifs)
    {
        ...
        while (ifs >> a >> b >> c >> d)
        {
            ...
            m_precodingmatrix[i][j++] = a;
            m_precodingmatrix[i][j++] = b;
            m_precodingmatrix[i][j++] = c;
            m_precodingmatrix[i][j++] = d;
            i++;
        }
        ifs.close();
    }
    //std::cout << "Precoding matrix:" << std::endl;
    //std::cout << m_precodingmatrix[0][0] << " " << m_precodingmatrix[0][1] << std::endl;
    //std::cout << m_precodingmatrix[1][0] << " " << m_precodingmatrix[1][1] << std::endl;
}
...
NrGnbPhy::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NrGnbPhy")
            ...
            .AddAttribute("UseRawDlData",
                           "If true. use raw DlData Api; else use XNEW Api.",
                           BooleanValue (true),
                           MakeBooleanAccessor (&NrGnbPhy::SetUseRawDlData),
                           MakeBooleanChecker()) //zhongwei: add this attribute to choose to use XNEW multi-data scheme or not
...
NrGnbPhy::GetBeamConfId(uint16_t rnti) const
{
    ...
    for (std::size_t i = 0; i < m_deviceMap.size(); i++)
    {
        ...
        if (ueRnti == rnti)
        {
            ...
            BeamId beamId1 = m_spectrumPhys[0]->GetBeamManager()->GetBeamId(m_deviceMap.at(i));
            BeamId beamId2 = BeamId::GetEmptyBeamId();
            BeamId beamId3 = BeamId::GetEmptyBeamId();
            BeamId beamId4 = BeamId::GetEmptyBeamId();
            if (m_spectrumPhys.size() > 1)
            {
                m_spectrumPhys[1]->GetBeamManager()->GetBeamId(m_deviceMap.at(i));
                m_spectrumPhys[2]->GetBeamManager()->GetBeamId(m_deviceMap.at(i));
                m_spectrumPhys[3]->GetBeamManager()->GetBeamId(m_deviceMap.at(i));
            }
            return BeamConfId(beamId1, beamId2, beamId3, beamId4);
        }
    }
    return BeamConfId(BeamId(0, 0), BeamId::GetEmptyBeamId());
}
...
Time
NrGnbPhy::DlMultiData (const std::shared_ptr<DciInfoElementTdma> &dci)
{
    NS_LOG_FUNCTION (this);
    ...
    std::vector< Ptr<PacketBurst>> m_pktBurst;
    std::vector<int> m_streamIndex;
    for (uint8_t streamIndex = 0; streamIndex < m_spectrumPhys.size(); streamIndex++)
    {
        ...
        m_pktBurst.push_back(pktBurst); //zhongwei: put together multi pksBursts
        ...
        m_streamIndex.push_back(streamIndex); //zhongwei: streamIndex also need to be put together
    }

       //pengpeng: push streamIndex 2&3 temporarily
    m_streamIndex.push_back(2);
    m_streamIndex.push_back(3);

    //zhongwei: here we newly create NrGnbPhy::SendMultiDataChannels(), in which two parameters, i.e. m_pktBurst and m_streamIndex have been changed comparing with the NR source code
    Simulator::Schedule (NanoSeconds (1.0), &NrGnbPhy::SendMultiDataChannels, this, m_pktBurst, varTtiPeriod - NanoSeconds (2.0), dci, m_streamIndex);
    return varTtiPeriod;
}
...
NrGnbPhy::StartVarTti(const std::shared_ptr<DciInfoElementTdma>& dci)
{
    ...
    else if (dci->m_type == DciInfoElementTdma::DATA)
    {
        if (dci->m_format == DciInfoElementTdma::DL)
        {
            //zhongwei: set this api to choose use multi-data scheme or not. If true. use raw DlData Api; else use multi-data Api.
            //more details about this flag: anyone condition satisfies these three requirements will lead to switch to using LENA scheme
            //first condition: m_UseRawDlData = true, the flag we have configured in demo
            //second condition: dci->m_tbSize.size() == 1, which means Downlink Control Information only scheduled one stream
            //third condition: (dci->m_tbSize.size() == 2 ) && (dci->m_tbSize[0] == 0 ||dci->m_tbSize[1] == 0 ), which means Downlink Control Information has scheduled two streams but one of them is empty
            if (m_UseRawDlData || dci->m_tbSize.size() == 1 || ((dci->m_tbSize.size() == 2 ) && (dci->m_tbSize[0] == 0 ||dci->m_tbSize[1] == 0 ))) 
            {
                varTtiPeriod = DlData (dci); //zhongwei: LENA scheme
            }
            else
            {
                varTtiPeriod = DlMultiData (dci); //zhongwei: multi-data XNEW scheme
            }
...
void
NrGnbPhy::SetChannel (Ptr<SpectrumChannel> c)
{
    m_channel = DynamicCast<MultiModelSpectrumChannel>(c);
}

void
NrGnbPhy::SendMultiDataChannels (const std::vector< Ptr<PacketBurst>> &m_pb, const Time &varTtiPeriod,
                                 const std::shared_ptr<DciInfoElementTdma> &dci,
                                 const std::vector<int> &m_streamIndex)
{
    NS_LOG_FUNCTION (this);
    // update beamforming vectors (currently supports 1 user only)
    bool found = false;
    std::vector<Ptr<MultiStreamPhy>> uePhys;
    for (uint8_t i = 0; i < m_deviceMap.size (); i++)
    {
        Ptr<NrUeNetDevice> ueDev = DynamicCast<NrUeNetDevice> (m_deviceMap.at (i));
        Ptr<NrUePhy> uePhy = (DynamicCast<NrUePhy>(ueDev->GetPhy (GetBwpId ())));
        uint64_t ueRnti = uePhy->GetRnti ();
        //NS_LOG_INFO ("Scheduled rnti:"<<rnti <<" ue rnti:"<< ueRnti);
        if (dci->m_rnti == ueRnti)
        {
            uePhys.push_back(DynamicCast<MultiStreamPhy>(uePhy));
            ChangeBeamformingVector(m_deviceMap.at (i));
            found = true;
            break;
        }
    ...
    //zhongwei: initialize m_txParams to put txParams together in
    std::vector<Ptr<SpectrumSignalParameters>> m_txParams;
  
    //pengpeng: to test 4-size params, set firstly range from 0 - 1 to avoid crash.    
    // for(uint8_t streamIndex = 0; streamIndex < m_streamIndex.size(); streamIndex++)
    for(uint8_t streamIndex = 0; streamIndex < 2; streamIndex++)
    {
        //zhongwei: get each stream's txParams according to streamIndex, then put them together into m_txParams
        Ptr<SpectrumSignalParameters> txParam = m_spectrumPhys.at(streamIndex)->StartTxMultiDataFrames(m_pb.at(streamIndex), ctrlMsgs, varTtiPeriod);
        //pengpeng:push twice to get testing params
        m_txParams.push_back(txParam);
        m_txParams.push_back(txParam);
    }
  
    // pengpeng: Test for 4x4 SU-MIMO scheme in phy layer by duplicating txParams
    // Ptr<SpectrumSignalParameters> txParam_2 =
    //     m_spectrumPhys.at(0)->StartTxMultiDataFrames(m_pb.at(0), ctrlMsgs, varTtiPeriod);
    // Ptr<SpectrumSignalParameters> txParam_3 =
    //     m_spectrumPhys.at(1)->StartTxMultiDataFrames(m_pb.at(1), ctrlMsgs, varTtiPeriod);
    // m_txParams.push_back(txParam_2);
    // m_txParams.push_back(txParam_3);

    //zhongwei: judge if m_channel and StartMultiTx multi txParams
    if(m_channel)
    {
        m_channel->StartMultiTx(m_txParams, this, uePhys, m_precodingmatrix);
    }
    else
    {
    ...
    for(uint8_t streamIndex = 0; streamIndex < m_streamIndex.size(); streamIndex++)
    {
        m_spectrumPhys.at(streamIndex)->EndMultiDataTx(varTtiPeriod);
    }
...
void
NrGnbPhy::SetUseRawDlData (bool UseRawDlData)
{
    NS_LOG_FUNCTION (this << UseRawDlData);
    m_UseRawDlData = UseRawDlData;
}

void
NrGnbPhy::StartMultiRx(std::vector<Ptr<SpectrumSignalParameters>> m_txParams, Ptr<MultiStreamPhy> gnbPhy, complex3DVector_t total_G, complex2DVector_t precodingmatrix)
{
    // TODO: support uplink multi-stream
}
