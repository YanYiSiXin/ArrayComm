    ...
    static TypeId tid =
        TypeId("ns3::NrMacSchedulerNs3")
            ...
            .AddAttribute("EnableHarqReTx",
                          ...
                          MakeBooleanChecker())
            .AddAttribute ("UseRawFSlotStructure",
                           "xiaoying : If true. use  the original symbol distribution of slot F is maintained; otherwise use XNEW scheme.",
                           BooleanValue (true),
                           MakeBooleanAccessor (&NrMacSchedulerNs3::SetUseRawFSlotStructure),
                           MakeBooleanChecker()); //xiaoying : if false ,use configure XNEW scheme to design F slot
...
void
NrMacSchedulerNs3::SetUseRawFSlotStructure (bool UseRawFSlotStructure)
{
    m_UseRawFSlotStructure = UseRawFSlotStructure;
}
                              ...
                              const std::vector<DlHarqInfo>& dlHarqFeedback)
{
  ...
  //xiaoying : Add function input parameter ¡°Slot type¡±
                                ...
                                LteNrTddSlotType type)
{
    ...
    //xiaoying : if m_UseRawFSlotStructure is true,use LENA scheme,otherwise use XNEW scheme which only supports two F slot structures:
    //1 DL Ctrl ,9 DL DATA ,2 GAPs(or 3 GAPs),1 SRS(or not),1 UL Ctrl
    if (m_UseRawFSlotStructure==true)
    {
    ...
    {
        if (type == LteNrTddSlotType::F)
        { // xiaoying:if it's a type F, we have to consider DL CTRL symbols and DL DATA symbols(1+9), otherwise, don't
            dataSymPerSlot -= (m_dlCtrlSymbols+9);
        }
    }	
    ...
    // go backward.
    uint8_t lastSym = m_macSchedSapUser->GetSymbolsPerSlot () - m_ulCtrlSymbols; //Subtract one UL Ctrl symbol,
    PointInFTPlane ulAssignationStartPoint(0, lastSym);
    uint8_t ulSymAvail = dataSymPerSlot;
    //xiaoying:The F slot put GAP in the last three or four symbols
    if ((m_enableSrsInFSlots == true && type == LteNrTddSlotType::F&&m_UseRawFSlotStructure==false) )
    { 
      //In the F slot, the downlink symbol type is allocated first. According to the XNEW scheme, the first 
	    //10 symbols in the F slot are downlink type symbols.
	    ulSymAvail = dataSymPerSlot - 2; //Subtract two GAPs,and 1 symbol left.
    }
    ...
    { // SRS are included in F slots, and in UL slots if m_enableSrsInUlSlots=true
        ...
        ulSymAvail -= srsSym;
        //xiaoying : if F slot without SRS, put GAP
        if (srsSym==0 && type == LteNrTddSlotType::F&&m_UseRawFSlotStructure==false)
        {
	        ulSymAvail-=m_srsCtrlSymbols;	
        }
    }
    ...
    //xiaoying : Because the XNEW Schema F slot has an assigned GAP,totUlSym 
    //does not equal(dataSymPerSlot + m_ulCtrlSymbols) - ulSymAvail
    if (m_UseRawFSlotStructure)
    {
                                ...
                                LteNrTddSlotType type)
{
    ...
    //xiaoying : The first ten symbols of the F slot are DL CTRL and DL DATA in the XNEW scheme
    if ((m_enableSrsInFSlots == true && type == LteNrTddSlotType::F&&m_UseRawFSlotStructure==false) )
    { 
         dlSymAvail = dataSymPerSlot - ulAllocations.m_totUlSym-3; //3 GAPs 
    }
    if ( ulAllocations.m_totUlSym==2 && type==LteNrTddSlotType::F&&m_UseRawFSlotStructure==false) 
    {
        dlSymAvail = dataSymPerSlot - ulAllocations.m_totUlSym-3 + m_srsCtrlSymbols;//2 GAPs ,1 srs
    }
