...
class NrSpectrumPhy : public SpectrumPhy
{
  public:
    ...
    typedef void(* StreamTracedCallback)(uint8_t, uint8_t, Ptr<const SpectrumValue>, Ptr<const SpectrumValue> );
    /**
    ...
    void SetUnlicensedMode(bool unlicensedMode);
      /**
     * \brief sets UseRawRbSymToCqi.If UseRawRbSymToCqi is true, the number of symbols corresponding to RB number when calculating CQI is the number of DATA type symbols that match the current time slot. Otherwise, not
     * \param UseRawRbSymToCqi flag 
     */
    void SetUseRawRbSymToCqi (bool UseRawRbSymToCqi);
  /**
  ...
  /**
   * \brief Starts transmission of multi data frames on connected spectrum channel object, return each stream's txParams
   * \param pb packet burst to be transmitted
   * \param ctrlMsgList control message list
   * \param duration the duration of transmission
   */
    Ptr<SpectrumSignalParameters> StartTxMultiDataFrames (const Ptr<PacketBurst>& pb, const std::list<Ptr<NrControlMessage> >& ctrlMsgList, Time duration);
   ...
   */
    void EndMultiDataTx (Time duration);
  ...
  private:
    ...
    TracedCallback<uint8_t, uint8_t, Ptr<const SpectrumValue>, Ptr<const SpectrumValue>  > m_hbfStreamTrace;
    ...
    uint32_t m_symnum {1} ; //xiaoying:current slot DL/UL DATA type symbols
    bool m_UseRawRbSymToCqi {true} ; //xiaoying:BUG3:If m_UseRawRbSymToCqi is true, the number of symbols corresponding to RB number when calculating CQI is the number of DATA type symbols that match the current time slot. Otherwise, not.
};
