...
class NrGnbPhy : public NrPhy
{
  ...
  public:
    /**
  * \xiaoying:GenerateStructuresFromPattern() public
   ...
   */
    ...
    /**
     * \zhongwei: brief Set the channel to the Gnb Device
     * \param the SpectrumChannel attached to this Device
     * \now the SpectrumChannel is attached to the Gnb Device so that we can use Gnb Device to send multi-stream data directly
     */
     ...
     */
    Ptr<SpectrumValue> GetNoisePowerSpectralDensityPub ();
    /**
     * \zhongwei: brief used to receive multi stream signals in devices, where equalization being processed.
     */
    void StartMultiRx(std::vector<Ptr<SpectrumSignalParameters>> m_txParams, Ptr<MultiStreamPhy> gnbPhy, complex3DVector_t total_G, complex2DVector_t precodingmatrix);
  protected:
    /**
  ...
  private:
    ...
    /**
    * \brief Transmit the data stored in multi pbs to the spectrum phy 
    *
    * \param m_pb Data to transmit
    * \param varTtiPeriod period of transmission
    * \param dci DCI of the transmission
    * \param m_streamIndex The ids of the streams, which identify the instance of the
    *        NrSpecturmPhy to be used to transmit the packet burst
    */
    void SendMultiDataChannels (const std::vector< Ptr<PacketBurst>> &m_pb, const Time &varTtiPeriod, 
                              const std::shared_ptr<DciInfoElementTdma> &dci, const std::vector<int> &m_streamIndex);
  ...
  /**
    ...
    * \return the time at which the transmission of DL data will end
    *
    * The method will get the data to transmit, and call SendMultiDataChannels.
    *
    * \see SendMultiDataChannels
    */
    Time DlMultiData (const std::shared_ptr<DciInfoElementTdma> &dci) __attribute__((warn_unused_result));
    ...
    void FillTheEvent();
    /**
     * \zhongwei:brief whether to use multi-data scheme.
     * \multi-data scheme api
     */
    void SetUseRawDlData (bool UseRawDlData);
    /**
     * \zhongwei: brief initialize the precoding matrix.
     */
    void InitPrecodingMatrix();
  private:
    ...
    uint32_t m_n1Delay{0}; //!< minimum processing delay (in slots) from the end of DL Data reception to the
    ...
    Ptr<MultiModelSpectrumChannel> m_channel {nullptr}; //zhongwei: channel is needed to be connected to the GnbPhy to start transmission StartMultiTx
    bool m_UseRawDlData {true}; //zhongwei: If true use raw DlData Api; else use XNEW Api
    complex2DVector_t m_precodingmatrix;  //zhongwei: the precoding matrix
};
