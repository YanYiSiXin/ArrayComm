...
NrAmc::CreateCqiFeedbackWbTdma(const SpectrumValue& sinr, uint8_t& mcs) const
{
    ...
    else if (m_amcModel == ErrorModel)
    {
        ...
        while (mcs <= m_errorModel->GetMaxMcs())
        {
		  //xiaoying: MCS selection/CQI calculation: modify 
		  //the input TB size to be consistent with the actual
		  // maximum TTI corresponding TB size.
...
uint8_t
NrAmc::CreateCqiFeedbackWbTdmaNew(const SpectrumValue& sinr, uint8_t& mcs,uint32_t symnum) const
{
    NS_LOG_FUNCTION(this);
    ...
    else if (m_amcModel == ErrorModel)
    {
        ...
        while (mcs <= m_errorModel->GetMaxMcs())
        {
		  ...
		  // maximum TTI corresponding TB size.
            output = m_errorModel->GetTbDecodificationStats(sinr,
                                                            rbMap,
                                                            CalculateTbSize(mcs, rbMap.size()*symnum),
                                                            mcs,
                                                            NrErrorModel::NrErrorModelHistory());
            if (output->m_tbler > 0.1)
            {
