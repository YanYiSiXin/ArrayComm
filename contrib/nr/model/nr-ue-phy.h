...
class NrUePhy : public NrPhy
{
  ...
  public:
    ...
    /**
   * \brief Generate a DL CQI report in XNEW
   *
   * Connected by the helper to a callback in corresponding ChunkProcessor
   *
   * \param sinr the SINR
   * \param streamIndex the index of the stream for which is reported this SINR
   */
  void GenerateDlCqiReportNew (const SpectrumValue& sinr, uint8_t streamIndex ,uint32_t symnum);
  ...
  private:
    ...
    /**
   * \zhongwei: brief this part realize the MMSE equalization
   */
    complex2DVector_t MmseCholesky (complex2DVector_t matrixH, double m_noisePowerSpectralDensity);
    complex2DVector_t MmseSolveSimplified_down(complex2DVector_t matrixL, complex2DVector_t aux1); //zhongwei: calculate the lower-triangular matrix multiplication
    complex2DVector_t MmseSolveSimplified_up(complex2DVector_t matrixL, complex2DVector_t aux1); //zhongwei: calculate the upper-triangular matrix multiplication
    complex2DVector_t MmseSolve (complex2DVector_t matrixH, double m_noisePowerSpectralDensity);
    //pengpeng: achieve ZF Precoding
    complex2DVector_t ZFSolve(complex2DVector_t matrixH);
    ...
    bool m_reportedRi2{false}; /**< Flag to keep track of an event when a UE
                                    ...
                                    */
    bool useRawCqiMethod {true};
    std::vector<uint8_t> twostream_cqi; // Single stream CQI vector 
    std::vector<uint8_t> onestream_cqi; // double stream CQI vector 
    typedef void (* RsrpracedCallback)(uint8_t, double);
    TracedCallback<uint8_t, double > m_RSRPTrace;
};
