...
NrPhy::InstallSpectrumPhy(const Ptr<NrSpectrumPhy>& spectrumPhy)
{
    ...
    //zhongwei: here we DynamicCast the NrSpectrumPhy into SpectrumPhy
    //and push them into base_spectrumPhys so that to avoid the introduce of NR in ns3 core code
    Ptr<SpectrumPhy> base_spectrumPhy = DynamicCast<SpectrumPhy> (spectrumPhy);
    base_spectrumPhys.push_back(base_spectrumPhy);
