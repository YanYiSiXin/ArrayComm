    ...
    static TypeId tid =
        TypeId("ns3::NrUePhy")
            ...
            .AddAttribute("UseFixedRi",
                          ...
                          MakeBooleanChecker())
            .AddAttribute ("UseRawCqiMethod",
                           "use raw cqi method",
                           BooleanValue (true),
                           MakeBooleanAccessor (&NrUePhy::useRawCqiMethod),
                           MakeBooleanChecker ())
            .AddAttribute (
                "RiSinrThreshold1",
            ...
            .AddTraceSource ("Rsrp",
                             "Rs Received Power.",
                             MakeTraceSourceAccessor (&NrUePhy::m_RSRPTrace),
                             "ns3::NrUePhy::RsrpracedCallback");
    return tid;
}

complex2DVector_t
NrUePhy::MmseCholesky (complex2DVector_t matrixH, double inv_snr)
{
    //zhongwei: This method obtains the cholesky decomposition of the positive definite hermitian matrix M=(H'H+NoI),
    //where we have left the input matrixH in the format H
    //This method is a C++ adaptation of the cholesky decompositoin examples provided in https://rosettacode.org/wiki/Cholesky_decomposition
    ...
    {
        //zhongwei: initialize the result matrix
    ...
    {
        for ( uint16_t kdiag = 0 ; kdiag < (kcol + 1) ; kdiag ++) //zhongwei: equality reached in loop end
        {
            //zhongwei: first we build the necessary coefficient of Hermitian matrix M(kcol,kdiag) = H'H+I
            std::complex<double> sum = ( kcol == kdiag )? inv_snr : 0;
            for ( uint16_t sumiter = 0 ; sumiter <  matrixH.size() ; sumiter ++)
            {
                sum += std::conj( matrixH.at(sumiter).at(kcol) ) * matrixH.at(sumiter).at(kdiag) ;
            }
        //zhongwei: second we apply the negative sumatorium of the normal Cholesky algorithm in the accumulator variable 'sum'
        ...
        //zhongwei: finally we update the matrix
...
complex2DVector_t
NrUePhy::MmseSolveSimplified_down(complex2DVector_t matrixL, complex2DVector_t aux1)
{
    //zhongwei: split matrix aux1 into several vectors here as aux11
    complexVector_t aux11(aux1.size(), std::complex<double>(0.0, 0.0)); 
    //zhongwei: define matrix aux3 to deposit the results of matrixL*aux22=aux11��i.e. complete the calculation of matrixL*aux3=aux1
    complex2DVector_t aux3;

    for (uint16_t i  = 0; i < aux1.at(0).size(); i++)
    {
        for (uint16_t j =  0; j < aux1.size(); j++)
        {
            aux11.at(j) = aux1.at(j).at(i);
        }
        complexVector_t aux22;
        //zhongwei: calculate matrixL*aux22=aux11
        for (uint16_t row = 0; row < matrixL.size() ; row ++)
        {
            std::complex<double> sum = aux11.at(row);
            for (uint16_t col = 0; col < row ; col ++)
            {
                sum -= aux22.at(col) * matrixL.at(row).at(col);
            }
            aux22.push_back( sum /  matrixL.at(row).at(row) );
        }
        //zhongwei: push the aux22 into aux3
        aux3.push_back(aux22);
    }

    //zhongwei: transpose the aux3 and obtain the final result
    complex2DVector_t aux33(aux3.at(0).size(), complexVector_t(aux3.size()));
    for (uint16_t row = 0; row < aux3.size() ; row ++)
    {
        for (uint16_t col = 0; col < aux3.at(0).size() ; col ++) //zhongwei: col-row indexes inverted here for Hermitian matrix
        {
            aux33.at(col).at(row) = aux3.at(row).at(col);
        }
    }
    return aux33;
}
...
complex2DVector_t
NrUePhy::MmseSolveSimplified_up(complex2DVector_t matrixL, complex2DVector_t aux1)
{
    //zhongwei: split matrix aux1 into several vectors here as aux11
    complexVector_t aux11(aux1.size(), std::complex<double>(0.0, 0.0)); 
    //zhongwei: define matrix aux3 to deposit the results of matrixL*aux22=aux11��i.e. complete the calculation of matrixL*aux3=aux1
    ...
    for (uint16_t i  = 0; i < aux1.at(0).size(); i++)
    {
        ...
        // solve L'*x=aux2. We preallocate initialized with zeros because we start writing at the end of the vector
        complexVector_t aux22( matrixL.size() , 0.0 );
        for (uint16_t revRow = 0; revRow < matrixL.size(); revRow ++ )
        {
            ...
            {
                sum -= aux22.at(col) * std::conj(matrixL.at(col).at(row));//col-row indexes inverted for Hermitian matrix L'
            }
            aux22[row] = ( sum /  matrixL.at(row).at(row) );
        }
        //zhongwei: push the aux22 into aux3
        aux3.push_back(aux22);
    }
...
complex2DVector_t
NrUePhy::MmseSolve (complex2DVector_t matrixH, double inv_snr)
{
    //zhongwei: Cholesky linear solver for x of linear system (H'*H+No*I)x=H'v, returning x=(H'*H+No*I)^-1H'
    ...
    //zhongwei: first we get the Hermitian matrix of H
    complex2DVector_t aux1(matrixH.at(0).size(), complexVector_t(matrixH.size()));
    for (uint16_t col = 0; col < matrixH.at(0).size() ; col ++)
    {
        for (uint16_t row = 0; row < matrixH.size() ; row ++) //zhongwei: col-row indexes inverted here for Hermitian matrix
        {
            aux1.at(col).at(row) = std::conj( matrixH.at(row).at(col));
        }
    }

    //zhongwei: get the matrix L, in which L*L'=M��M = H'H+I
    complex2DVector_t matrixL=MmseCholesky(matrixH, inv_snr); //zhongwei: define M = H'H+I, factorization M=L*L' with lower-triangular L

    //zhongwei: get the matrix aux2, in which L*aux2 = aux1
    complex2DVector_t aux2 = MmseSolveSimplified_down(matrixL, aux1);//zhongwei: define L'x=aux2, solve L*aux2 = aux1,

    //zhongwei: get the final result matrix x, in which L'x=aux2
    complex2DVector_t x = MmseSolveSimplified_up(matrixL, aux2);
    return( x );
}

//pengpeng: consruct ZF solver using MmseSolver as the access point
complex2DVector_t
NrUePhy::ZFSolve(complex2DVector_t matrixH)
{
    //pengpeng: The proximity to 0 indicates the ZF accuracy, though you should avoid using 0.
    return NrUePhy::MmseSolve(matrixH, 1e-22);
}
...
NrUePhy::GenerateDlCqiReport(const SpectrumValue& sinr, uint8_t streamId)
{
    ...
    if (m_ulConfigured && (m_rnti > 0) && m_receptionEnabled)
    {
        ...
        //xiaoying : Single-stream and dual-stream CQI vectors are initialized and managed separately.
        if (onestream_cqi.empty ()) // No DL CQI reported yet, initialize the vector
        {
            // Remember, scheduler uses MCS 0 for CQI 0.
            // See, NrMacSchedulerCQIManagement::DlWBCQIReported
            onestream_cqi = std::vector <uint8_t> (m_spectrumPhys.size (), 0);
            twostream_cqi = std::vector <uint8_t> (m_spectrumPhys.size (), 0);
        ...
        NS_ASSERT (streamId < onestream_cqi.size ());
        ...
        //xiaoying : After the downlink dual-stream retransmits the single-stream,
        // the next TB dual-stream uses the CQI of the previous dual-stream .
        Values::const_iterator it;
        uint32_t rbId = 0;
        for (it = sinr.ConstValuesBegin (); it != sinr.ConstValuesEnd (); it++)
        {
        ...
        if (m_activeDlDataStreams==1)
        {
            onestream_cqi [streamId] = wbCqi;
        }
        else if (m_activeDlDataStreams==2 && sinr.GetValuesN()==rbId)//second condition: except harq rv=3 rb reduce to produce good channel sinr
        {
            twostream_cqi [streamId] = wbCqi;
        }
        ...
        if (m_dlCqiFeedbackCounter == m_activeDlDataStreams)
        {
            ...
            else
            {
                if (m_activeDlDataStreams==1)
                {
                    dlcqi.m_wbCqi = onestream_cqi;
                }
                if (dlcqi.m_ri==2)
                {
                    dlcqi.m_wbCqi = twostream_cqi;
                }
            }
...
void
NrUePhy::GenerateDlCqiReportNew (const SpectrumValue& sinr, uint8_t streamId ,uint32_t symnum)
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_ulConfigured && (m_rnti > 0) && m_receptionEnabled)
    {
      ...
      uint8_t mcs; // it is initialized by AMC in the following call
      uint8_t wbCqi = m_amc->CreateCqiFeedbackWbTdmaNew (sinr, mcs ,symnum);
                              ...
                              uint8_t rv)
{
    ...
    // XNEW: we find nr 2.3 module adapt m_activeDlDataStreams to m_activeDlDataStreamsPerHarqId, 
    // but this adaptation makes stream 1 has no retransmittion. So we go back to former way.
                               ...
                               [[maybe_unused]] uint8_t streamIndex)
{
    ...
    m_RSRPTrace (streamIndex, m_rsrp);
...
void
NrUePhy::StartMultiRx(std::vector<Ptr<SpectrumSignalParameters>> m_txParams, Ptr<MultiStreamPhy> gnbPhy, complex3DVector_t total_G, complex2DVector_t precodingmatrix)
{

    //zhongwei: select any txparams to traverse each subcarrier sequentially, and take out the corresponding 2X2 channel matrix G for digital and equalization processing
    auto txIteratorParams_1 = m_txParams.begin ();
    //zhongwei: txpsd iterator
    auto vit_1 = (*txIteratorParams_1)->psd->ValuesBegin ();
    //zhongwei: counter of the number of RBs
    uint8_t vitIndex_1 = 0;

    //zhongwei: initialize the matrix total_GP to be equalized with txpsd, i.e. the superposition of GP upon all RBs
    complex3DVector_t total_GP;

    uint8_t rxSpectrumPhysSize = total_G.size();
    uint8_t txSpectrumPhysSize = total_G.at(0).size();

    //zhongwei: get the bottom noise Nt
    Ptr<SpectrumValue> Nt = GetNoisePowerSpectralDensity();
    //zhongwei: noise iterator
    auto nit_1 = Nt->ValuesBegin();
    //zhongwei: initialize the matrix total_E to be equalized with noise next, i.e. the superposition of equalization matrix E upon all RBs
    complex3DVector_t total_E;
    //zhongwei: initialize the matrix total_EGP to be equalized with txpsd next, i.e. the superposition of matrix EGP upon all RBs
    complex3DVector_t total_EGP;

     //zhongwei: traverse each RB
    while (vit_1 != (*txIteratorParams_1)->psd->ValuesEnd ())
    {
        //zhongwei: initialize each G and its origin dimension
        complex2DVector_t G;
        G.resize(rxSpectrumPhysSize);
        for (int i = 0; i < rxSpectrumPhysSize; i++)
        {
            G[i].resize(txSpectrumPhysSize);
        }

        //zhongwei: initialize each GP and its origin dimension
        complex2DVector_t GP;
        GP.resize(rxSpectrumPhysSize);
        for (int i = 0; i < rxSpectrumPhysSize; i++)
        {
            GP[i].resize(txSpectrumPhysSize);
        }

        //zhongwei: initialize each EGP and its origin dimension
        complex2DVector_t EGP;
        EGP.resize(rxSpectrumPhysSize);
        for (int i = 0; i < rxSpectrumPhysSize; i++)
        {
            EGP[i].resize(txSpectrumPhysSize);
        }

        //zhongwei: sequentially extract the channel of each RB to form the 2x2 channel matrix G of a single RB
        for(uint8_t k = 0; k < total_G.size(); ++k)
        {
            for(uint8_t l = 0; l < total_G.at(k).size(); ++l)
            {
                G[k][l] = total_G[k][l][vitIndex_1];
            }
        }

        //pengpeng: calculating pseudo matrix of G to fill out the Precoding matrix
        precodingmatrix = ZFSolve(G);


        //zhongwei: digital processing of 2x2 channel matrix G
        for(uint8_t k = 0; k < G.size(); ++k)
        {
            for(uint8_t l = 0; l < precodingmatrix.at(0).size(); ++l)
            {
                for(uint8_t a = 0; a < G.at(0).size(); ++a)
                {
                    GP[k][l] += G.at(k).at(a) * precodingmatrix.at(a).at(l);
                    //pengpeng: Test the validity of ZF Process

                }
            }
        }

        //zhongwei: equalize the GP matrix after digital processing
        //according to the formula, the inverse value of the SNR of the incoming signal is required, which is the ratio of the noise on each RB in a single RB to the power of the incoming signal
        double noisePsd = *nit_1;
        double txPsd = *vit_1;
        double lamda = noisePsd / txPsd;
        //zhongwei: call MmseSlove to calculate the equalization matrix E
        complex2DVector_t E = MmseSolve(GP, lamda);

        //zhongwei: calculate the matrix EGP
        for(uint8_t i = 0; i < E.size(); ++i)
        {
            for(uint8_t j = 0; j < GP.at(0).size(); ++j)
            {
                for(uint8_t a = 0; a < E.at(0).size(); ++a)
                {
                    EGP[i][j] += E.at(i).at(a) * GP.at(a).at(j);
                }
            }
        }

    //zhongwei: push the digital and equalized matrix and its product with the original matrix into the final total_ E and total_ EGP respectively
    //matrix total_E's dimension: [RBs txport rxport]
    //matrix total_EGP's dimension: [RBs txport rxport]
    total_E.push_back(E);
    total_EGP.push_back(EGP);
    vit_1++;
    nit_1++;
    vitIndex_1++;
    }
    std::vector<Ptr<SpectrumPhy>> ueSpectrumPhys = this->GetSpectrumPhys();
    std::vector<Ptr<SpectrumPhy>> gnbSpectrumPhys = gnbPhy->GetSpectrumPhys();

    //zhongwei: this part realize the equalization of noise
    uint8_t i = 0;
    for( auto ueSpectrumPhyIterator = ueSpectrumPhys.begin(); ueSpectrumPhyIterator != ueSpectrumPhys.end(); ++ueSpectrumPhyIterator)
    {
        ...
        for( auto gnbSpectrumPhyIterator = gnbSpectrumPhys.begin(); gnbSpectrumPhyIterator != gnbSpectrumPhys.end(); ++gnbSpectrumPhyIterator)
        {
            ...
            //zhongwei: initialize each stream_noise as 0.0
            Ptr<SpectrumValue> stream_noise = Copy<SpectrumValue> ((*(m_txParams.begin ()))->psd);
            *stream_noise = 0.0;
            for (uint8_t k = 0; k < total_E.at(0).size(); ++k)
            {
                //zhongwei: copy the bottom noise Nt value for subsequent processing (since each bottom noise is the same size, only one Nt is initialized here)
                Ptr<SpectrumValue> raw_noise = Copy<SpectrumValue> (Nt);
                auto nit_2 = raw_noise->ValuesBegin();
                uint8_t nitIndex_2 = 0;
                //zhongwei: traverse according to each RB, and multiply the noise value with the coefficients of matrix E
                while (nit_2 != raw_noise->ValuesEnd ())
                {
                    *nit_2 = (*nit_2) * (std::norm (total_E.at(nitIndex_2).at(i).at(j)));
                    nit_2++;
                    nitIndex_2++;
                }
                //zhongwei: the processed noise value is added to the stream_noise
                (*stream_noise) = (*stream_noise) + (*raw_noise);
            }
            //zhongwei: the noise value is passed into the corresponding interference module of each receiving stream
            Ptr<NrSpectrumPhy> rxNrPhy = DynamicCast<NrSpectrumPhy> (*ueSpectrumPhyIterator);
            (rxNrPhy->GetNrInterference ())->SetNoisePowerSpectralDensity (stream_noise);
    ...
    //zhongwei: counter, marking dimensions of rxstreams
    uint8_t k = 0;
    for ( auto ueSpectrumPhyIterator = ueSpectrumPhys.begin(); ueSpectrumPhyIterator != ueSpectrumPhys.end(); ++ueSpectrumPhyIterator)
    {
        //zhongwei: counter, marking dimensions of txstreams
        uint8_t l = 0;
        ...
        for ( auto gnbSpectrumPhyIterator = gnbSpectrumPhys.begin(); gnbSpectrumPhyIterator != gnbSpectrumPhys.end(); ++gnbSpectrumPhyIterator)
        {
            ...
            //zhongwei: traverse according to each RB, and multiply the txpsd value with the coefficients of matrix EHP
            auto vit_2 = rxParams->psd->ValuesBegin (); // psd iterator
            uint8_t vitIndex_2 = 0;
            while (vit_2 != rxParams->psd->ValuesEnd ())
            {
                *vit_2 = (*vit_2) * (std::norm (total_EGP.at(vitIndex_2).at(k).at(l)));
                vit_2++;
                vitIndex_2++;
            }
            (*ueSpectrumPhyIterator)->StartRx (rxParams);
            ++txIteratorParams;
            //zhongwei: step into the next txstream dimension
            ++l;
        }
        //zhongwei: step into the next rxstream dimension
        ++k;
    }
}
