...
class TestGnbMacNew : public NrGnbMac
{
public:
  static TypeId GetTypeId (void);
  TestGnbMacNew (const std::string &pattern);
  virtual ~TestGnbMacNew (void) override;
...
NS_OBJECT_ENSURE_REGISTERED (TestGnbMacNew);

TypeId
TestGnbMacNew::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::TestGnbMacNew")
    .SetParent<NrGnbMac> ()
...
TestGnbMacNew::TestGnbMacNew (const std::string &pattern)
{
...
void
TestGnbMacNew::DoSlotDlIndication (const SfnSf &sfnSf, LteNrTddSlotType type)
{
...
void
TestGnbMacNew::DoSlotUlIndication (const SfnSf &sfnSf, LteNrTddSlotType type)
{
  uint32_t pos = sfnSf.Normalize ();
  pos = pos % m_pattern.size ();
...
void
TestGnbMacNew::SetCurrentSfn (const SfnSf &sfnSf)
{
 ...
 */
class LtePhyPatternNewTestCase : public TestCase
{
public:
  /**
   ...
   */
  LtePhyPatternNewTestCase (const std::string &pattern, const std::string &name)
    : TestCase (name), m_pattern (pattern)
  {}
  ~LtePhyPatternNewTestCase ();
...
LtePhyPatternNewTestCase::~LtePhyPatternNewTestCase ()
{
  if (m_phy)
    {
      m_phy->Dispose ();
...
void
LtePhyPatternNewTestCase::DoRun ()
{
...
void
LtePhyPatternNewTestCase::StartSimu ()
{
...
Ptr<NrGnbPhy>
LtePhyPatternNewTestCase::CreatePhy (const Ptr<NrGnbMac> &mac) const
{
...
Ptr<NrGnbMac>
LtePhyPatternNewTestCase::CreateMac (const Ptr<NrMacScheduler> &sched) const
{
  Ptr<NrGnbMac> mac = CreateObject<TestGnbMacNew> (m_pattern);
...
void
LtePhyPatternNewTestCase::Print (const std::string &msg1, const std::string& msg2,
                              const std::map<uint32_t, std::vector<uint32_t> > &str)
{
...
class NrLtePatternsNewTestSuite : public TestSuite
{
public:
  NrLtePatternsNewTestSuite () : TestSuite ("nr-phy-patterns-new", UNIT)
  {

    AddTestCase (new LtePhyPatternNewTestCase ("DL|DL|DL|F|UL|",
                                            "XNEW TDD Pattern NR test"), QUICK);

  }
};

static NrLtePatternsNewTestSuite nrLtePatternsNewTestSuite;
