    ...
    const ns3::Vector& uSpeed) const
{
    ...
    std::vector<std::complex<double>> channel = CalcComplexChannel(txPsd, longTerm, channelMatrix, channelParams, sSpeed, uSpeed);
    ...
    while (vit != tempPsd->ValuesEnd ())
    {
        if ((*vit) != 0.00)
        {
            std::complex<double> subsbandGain = channel.at(sbitIndex);
            sbitIndex++;
...
ThreeGppSpectrumPropagationLossModel::CalcBeamformingGain (Ptr<SpectrumValue> txPsd,
                                                           ...
                                                           Ptr<const MatrixBasedChannelModel::ChannelMatrix> channelMatrix,
                                                           Ptr<const MatrixBasedChannelModel::ChannelParams> channelParams,
                                                           const ns3::Vector &sSpeed, const ns3::Vector &uSpeed,
                                                           std::vector< std::complex<double> > &H_port_f) const
{
    H_port_f = CalcComplexChannel(txPsd, longTerm, channelMatrix, channelParams, sSpeed, uSpeed);
    Ptr<SpectrumValue> tempPsd = Copy<SpectrumValue> (txPsd);
    ...
    while (vit != tempPsd->ValuesEnd ())
    {
        if ((*vit) != 0.00)
        {
            *vit = (*vit) * (norm (H_port_f.at(sbitIndex)));
            sbitIndex++;
        }
        vit++;
    }
...
ThreeGppSpectrumPropagationLossModel::CalcComplexChannel (Ptr<SpectrumValue> txPsd,
                                                          PhasedArrayModel::ComplexVector longTerm,
                                                          Ptr<const MatrixBasedChannelModel::ChannelMatrix> channelMatrix,
                                                          Ptr<const MatrixBasedChannelModel::ChannelParams> channelParams,
                                                          const ns3::Vector &sSpeed, const ns3::Vector &uSpeed) const
{
    ...
    //Ptr<SpectrumValue> tempPsd = txPsd;
    ...
    std::vector<std::complex<double>> temp_complexchannel;

    // apply the doppler term and the propagation delay to the long term component
    // to obtain the beamforming gain
    auto vit = txPsd->ValuesBegin (); // psd iterator
    auto sbit = txPsd->ConstBandsBegin (); // band iterator
    while (vit != txPsd->ValuesEnd ())
    {
        ...
        //zhongwei: push each RB's susbandGain value into the temp_complexchannel and return
        temp_complexchannel.push_back(subsbandGain);
        vit++;
        sbit++;
    }
    return temp_complexchannel;
    ...
    Ptr<const PhasedArrayModel> bPhasedArrayModel) const
{
    ...
    std::vector<std::complex<double>> channel = DoCalcRxComplexChannel(rxPsd, a, b, aPhasedArrayModel, bPhasedArrayModel);

    auto vit = rxPsd->ValuesBegin (); // psd iterator
    //auto sbit = rxPsd->ConstBandsBegin (); // band iterator
    uint8_t sbitIndex = 0;
    while (vit != rxPsd->ValuesEnd ())
    {
        if ((*vit) != 0.00)
        {
            *vit = (*vit) * (norm (channel.at(sbitIndex)));
            sbitIndex++;
        }
        vit++;
    }
...
std::vector<std::complex<double>>
ThreeGppSpectrumPropagationLossModel::DoCalcRxComplexChannel (Ptr<SpectrumValue> txPsd,
                                                              Ptr<const MobilityModel> a,
                                                              Ptr<const MobilityModel> b,
                                                              Ptr<const PhasedArrayModel> aPhasedArrayModel,
                                                              Ptr<const PhasedArrayModel> bPhasedArrayModel) const
{
    ...
    std::vector<std::complex<double>> channel = CalcComplexChannel (txPsd, longTerm, channelMatrix, channelParams, a->GetVelocity (), b->GetVelocity ());

    //zhongwei: here return the complex channel being calculated to the MultiModelSpectrumChannel
    return channel;
