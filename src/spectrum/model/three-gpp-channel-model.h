...
//protected:
    ...
    struct ThreeGppChannelParams : public MatrixBasedChannelModel::ChannelParams
    {
        ...
        double m_ASA; //!< delay spread
        double m_ASD; //!< delay spread
        double m_ZSA; //!< delay spread
        double m_ZSD; //!< delay spread
        Double2DVector  m_rawAngle; //!< cluster angle angle[direction][n], where direction = 0(AOA), 1(ZOA), 2(AOD), 3(ZOD) in degree.
        ...
        DoubleVector clusterPowerForAngles; //!< cluster powers for angles
     ...
     */
    Ptr<ThreeGppChannelParams> GenerateSSFChannelParameters (Ptr<ThreeGppChannelParams> channelParams,
                                                             const Ptr<const ChannelCondition> channelCondition,
                                                             const Ptr<const ParamsTable> table3gpp,
                                                             const Ptr<const MobilityModel> aMob,
                                                             const Ptr<const MobilityModel> bMob) const;
    Ptr<ThreeGppChannelParams> m_GetParams (Ptr<const MobilityModel> aMob, 
                                            Ptr<const MobilityModel> bMob) ;
    ...
    Ptr<ChannelMatrix> GetNewAWGNChannel (const Ptr<const MobilityModel> sMob,
                                          const Ptr<const MobilityModel> uMob,
                                          Ptr<const PhasedArrayModel> sAntenna,
                                          Ptr<const PhasedArrayModel> uAntenna) const;
    /**
