...
PhasedArraySpectrumPropagationLossModel::CalcRxComplexChannel (Ptr<SpectrumValue> txPsd, Ptr<const MobilityModel> a, Ptr<const MobilityModel> b, Ptr<const PhasedArrayModel> aPhasedArrayModel, Ptr<const PhasedArrayModel> bPhasedArrayModel) const
{
  std::vector<std::complex<double>> complexchannel = DoCalcRxComplexChannel (txPsd, a, b, aPhasedArrayModel, bPhasedArrayModel);
  if (m_next != 0)
    {
      complexchannel = m_next->CalcRxComplexChannel (txPsd, a, b, aPhasedArrayModel, bPhasedArrayModel);
    }
  return complexchannel;
