    ...
    static TypeId tid =
        TypeId("ns3::UniformPlanarArray")
            ...
            .AddAttribute("PolSlantAngle",
                   ...
                   MakeDoubleChecker<double> (-M_PI, M_PI))
            .AddAttribute ("Yoffset",
                           "Subpanel y offset",
                           DoubleValue (0.0),
                           MakeDoubleAccessor (&UniformPlanarArray::SetYoffset),
                           MakeDoubleChecker<double> (0.0))
            .AddAttribute ("Zoffset",
                           "Subpanel z offset",
                           DoubleValue (0.0),
                           MakeDoubleAccessor (&UniformPlanarArray::SetZoffset),
                           MakeDoubleChecker<double> (0.0));
    return tid;
}
...
void
UniformPlanarArray::SetYoffset (double s)
{
    NS_LOG_FUNCTION (this << s);
    NS_ABORT_MSG_IF (s < 0, "Trying to set an invalid spacing: " << s);
    m_Yoffset = s;
...
double
UniformPlanarArray::GetYoffset (void) const
{
    return m_Yoffset;
}
void
UniformPlanarArray::SetZoffset (double s)
{
    NS_LOG_FUNCTION (this << s);
    NS_ABORT_MSG_IF (s < 0, "Trying to set an invalid spacing: " << s);
    m_Zoffset = s;
...
double
UniformPlanarArray::GetZoffset (void) const
{
    return m_Zoffset;
}
...
UniformPlanarArray::GetElementLocation(uint64_t index) const
{
    ...
    // plane.
    double xPrime = 0;
    double yPrime = m_Yoffset + m_disH * (index % m_numColumns);
    double zPrime = m_Zoffset + m_disV * floor (index / m_numColumns);
